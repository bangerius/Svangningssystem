...
Fjäderkraften $F_f$ är den kraft som motverkar en fjäders avskiljning ifrån sitt jämviktsläge $y-y_0$, beroende på fjäderns i fråga fjäderkonstant $k$.

\begin{equation}
F_f = -k(y - y_0)
\end{equation}

I vårt fall är det naturligt att dela upp fjäderkraften $F_f$ i normalfjäderkraften $F_{fn}$ och den resterande fjäderkraften $F_{fr}$. 
Vi låter $F_{fn}$ vara motriktad $F_g$ med samma storlek. Vi får då

\begin{equation}
F_{fn} = -mg = -kL
\end{equation}

där L är den utsträckning av fjädern som orsakas av en massa hängandes i fjädern.
Vi får då den resterande kraften $F_{fr} = F_f - F_{fn}$ som blir den resterande fjäderkraften vars jämnviktsläge varierar beroende på hur stor massa som hängs vid fjädern. I vårt system kan vi nu låta  $F_{fn}$ och $F_g$ ta ut varandra och med hjälp av hooks law säga att endast fjäderkraften $F_{fr} = -ky(t)$
...
\begin{equation}
 m\frac{d^2y(t)}{dt^2} =  -ky(t) -cm\frac{dy(t)}{dt} + x(t)
 \leftrightarrow  m\frac{d^2y(t)}{dt^2} + cm\frac{dy(t)}{dt} + ky(t) = x(t)
\end{equation}
...
\begin{equation}
m\frac{d^2y(t)}{dt^2} + cm\frac{dy(t)}{dt} + ky_1(t) = x_1(t)
\end{equation}

\begin{equation}
m\frac{d^2y(t)}{dt^2} + cm\frac{dy(t)}{dt} + ky_2(t) = x_2(t)
\end{equation}


Om vi då låter insignalen vara $a_1 x_1(t) + a_2 x_2(t)$ där $a_1$ och $a_2$ är reella konstanter så ger det:
\begin{equation}
m\frac{d^2y(t)}{dt^2} + cm\frac{dy(t)}{dt} + ky(t) = a_1 x_1(t) + a_2 x_2(t)
\end{equation}

Vilket leder till:

\begin{equation}
m\frac{d^2y(t)}{dt^2} + cm\frac{dy(t)}{dt} + ky(t) = a_1 (m\frac{d^2y_1(t)}{dt^2} + cm\frac{dy_1(t)}{dt} + ky_1(t) ) + a_2 (m\frac{d^2y_2(t)}{dt^2} + cm\frac{dy_2(t)}{dt} + ky_2(t))
\end{equation}
...
Alltså uppfyller systemet kravet för linjäritet.
